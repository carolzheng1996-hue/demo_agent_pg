...
def _load_engineered_dataframe(state: DGGlobalState) -> pd.DataFrame:
    if not bool(state.read("enable_feature_engineering")):
        profile = state.read("dataset_profile", {})
        formatted_paths = profile.get("formatted_dataset_paths") or {}
        if formatted_paths:
            frames = [pd.read_parquet(Path(str(path))) for path in formatted_paths.values()]
        ...
        return pd.read_parquet(Path(str(formatted_path)))
...
def _split_frames(engineered_df: pd.DataFrame, state: DGGlobalState) -> Tuple[pd.DataFrame, pd.DataFrame]:
    row_ids = state.read("split_strategy_result", {}).get("row_ids", {})
    if not row_ids:
        empty = engineered_df.iloc[0:0].copy().reset_index(drop=True)
        return engineered_df.copy().reset_index(drop=True), empty
...
def run(state: DGGlobalState) -> Dict:
    ...
    train_df, val_df = _split_frames(engineered_df, state)
    ...
    scaled_train, scaled_val, scaling = _normalize_with_train_statistics(
        train_df,
        val_df,
        numeric_cols,
