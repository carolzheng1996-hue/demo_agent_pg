...
def _extract_pending_approval(iterations: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    for iteration in reversed(iterations):
        for step in reversed(iteration.get("steps", [])):
            result = step.get("result") or {}
            proposal = result.get("proposal") or {}
            code = proposal.get("generated_code")
            if result.get("approved") is False and code:
                return {
                    "iteration_id": iteration.get("iteration_id"),
                    "subagent": step.get("step_name"),
                    "generated_code": code,
                    "proposal": proposal,
                    "result": result,
                }
...
def _pending_approval_artifact(task_id: str, pending_approval: Dict[str, Any]) -> Path:
    task_dir = OUTPUT_DIR / task_id
    iteration_id = str(pending_approval.get("iteration_id") or "")
    subagent = str(pending_approval.get("subagent") or "")
    return task_dir / iteration_id / subagent / "result.json"
...
def load_task_detail(task_id: str) -> Dict[str, Any]:
    ...
    pending_approval = _extract_pending_approval(iterations)
    final_summary = _read_text(task_dir / "final_summary.md")
    iteration_history = _read_json(task_dir / "iteration_history.json") or {}
    status = "awaiting_confirmation" if pending_approval else ("completed" if final_summary else "running")
...
class JobStore:
    """In-memory job registry for long-running agent tasks."""
...
class DGWebApplication:
    """Wrap orchestrator execution so the HTTP layer stays thin."""
    ...
    def approve_task(self, task_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        pending_approval = detail.get("pending_approval")
        if not pending_approval:
            raise ValueError("This task has no pending generated code to approve.")

        task_dir = OUTPUT_DIR / task_id
        original_payload = _read_json(task_dir / JOB_PAYLOAD_FILE)
        if not original_payload:
            raise FileNotFoundError(f"Missing {JOB_PAYLOAD_FILE} for task {task_id}")

        approval_state = _read_json(task_dir / APPROVAL_STATE_FILE) or {"approved_generated_codes": {}}
        approved_generated_codes = dict(approval_state.get("approved_generated_codes") or {})
        approved_generated_codes[pending_approval["subagent"]] = pending_approval["generated_code"]

        resumed_payload = {
            **original_payload,
            "approve_generated_code": bool(original_payload.get("approve_generated_code", False)),
            "resume_task_id": task_id,
            "approved_generated_codes": approved_generated_codes,
        }

        _write_json(task_dir / APPROVAL_STATE_FILE, {"approved_generated_codes": approved_generated_codes})
        return self.submit_job(resumed_payload)

    def modify_task_code(self, task_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        detail = load_task_detail(task_id)
        pending_approval = detail.get("pending_approval")
        if not pending_approval:
            raise ValueError("This task has no pending generated code to modify.")

        feedback = str(payload.get("feedback", "")).strip()
        if not feedback:
            raise ValueError("feedback is required")

        proposal = dict(pending_approval.get("proposal") or {})
        ...
        if not rewritten:
            raise ValueError(f"Failed to regenerate code from feedback for subagent {pending_approval.get('subagent')}.")
        ...
        artifact_path = _pending_approval_artifact(task_id, pending_approval)
        artifact_payload = _read_json(artifact_path) or {}
        artifact_payload["proposal"] = proposal
        artifact_payload["approved"] = False
        _write_json(artifact_path, artifact_payload)
        return {
            "task_id": task_id,
            "pending_approval": {
                **pending_approval,
                "generated_code": rewritten,
                "proposal": proposal,
            },
        }
    ...
    def _persist_task_metadata(self, task_id: Optional[str], payload: Dict[str, Any], state: DGGlobalState) -> None:
        if not task_id:
            return
        ...
        persisted_payload = {
            ...
            if key not in {"resume_task_id"}
        }
        _write_json(task_dir / JOB_PAYLOAD_FILE, persisted_payload)
        _write_json(
            task_dir / APPROVAL_STATE_FILE,
            {
                "approved_generated_codes": state.read("approved_generated_codes", {}) or payload.get("approved_generated_codes", {}) or {},
            },
        )
    ...
    def _run_job(self, job_id: str, payload: Dict[str, Any]) -> None:
        ...
        try:
            ensure_directories()
            initial_state = {
                "approve_generated_code": bool(payload.get("approve_generated_code")),
                ...
                "use_custom_processing": bool(payload.get("use_custom_processing")),
                "custom_processing_steps": str(payload.get("custom_processing_steps") or "").strip(),
                "input_length": payload.get("input_length"),
                "output_length": payload.get("output_length"),
                "time_increment": payload.get("time_increment"),
                "normalization_policy": str(payload.get("normalization_policy") or "auto"),
                "max_iterations": int(payload.get("max_iterations", 3)),
                "approved_generated_codes": payload.get("approved_generated_codes", {}) or {},
            }
            if payload.get("resume_task_id"):
                initial_state["task_id"] = str(payload["resume_task_id"])
            ...
            self._persist_task_metadata(task_id, payload, state)
        ...
        while not stop_event.wait(0.8):
            ...
            if task_id:
                try:
                    ...
                    self._persist_task_metadata(task_id, job_payload, state)
...
class DGRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        try:
            ...
            if parsed.path == "/api/config":
                self._send_json(
                    {
                        "defaults": self.app.default_form_values(),
    ...
    def do_POST(self) -> None:  # noqa: N802
        try:
            ...
            if parsed.path.startswith("/api/tasks/") and parsed.path.endswith("/approve"):
                task_id = parsed.path.removesuffix("/approve").rsplit("/", 1)[-1]
                payload = self._read_json_body()
                result = self.app.approve_task(task_id, payload)
                self._send_json(result, status=HTTPStatus.ACCEPTED)
                return

            if parsed.path.startswith("/api/tasks/") and parsed.path.endswith("/modify"):
                task_id = parsed.path.removesuffix("/modify").rsplit("/", 1)[-1]
                payload = self._read_json_body()
                result = self.app.modify_task_code(task_id, payload)
    ...
    def _normalize_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        normalized = {
            ...
            "approve_generated_code": bool(payload.get("approve_generated_code", False)),
            ...
            "use_custom_processing": bool(payload.get("use_custom_processing", False)),
            "custom_processing_steps": str(payload.get("custom_processing_steps", "")).strip(),
            ...
            "normalization_policy": str(payload.get("normalization_policy", "auto")).strip() or "auto",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="loongflow_DGagent web UI")
...
def main() -> None:
    ...
    print(f"Serving loongflow_DGagent UI on http://{args.host}:{args.port}")
